...
export class OrderRepository implements IOrderRepository {
	async saveOrder(db: any, orderId: string, order: any): Promise<void> {
		await db.prepare('INSERT OR REPLACE INTO orders (id, payload) VALUES (?, ?)').bind(orderId, JSON.stringify(order)).run();
	}

	async getOrderById(db: any, orderId: string): Promise<Order | null> {
		const row = (await db.prepare('SELECT id, status, payload, created_at FROM orders WHERE id = ?').bind(orderId).first()) as any;
