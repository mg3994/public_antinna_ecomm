...
export class AuthService implements IAuthService {
	...
	async verifyIdToken(token: string, projectId: string, kv: any): Promise<VerifiedUser | null> {
		...
		try {
			...
			const cryptoKey = await crypto.subtle.importKey('jwk', targetJwk, { name: 'RSASSA-PKCS1-v1_5', hash: 'SHA-256' }, false, ['verify']);

			const dataBytes = new TextEncoder().encode(`${parts[0]}.${parts[1]}`);
			const signatureBytes = this.base64UrlToUint8Array(parts[2]);

			const verified = await crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, signatureBytes, dataBytes);
