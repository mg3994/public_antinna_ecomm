...
import { UnauthorizedError, ForbiddenError, NotFoundError, ValidationError } from '../domain/exceptions';
...
export class CreateOrderUseCase {
	...
	async execute(db: any, kv: any, projectId: string, order: any, authHeader?: string): Promise<{ orderId: string; order: any }> {
...
export class GetOrdersUseCase {
	...
	async execute(
		...
		const filteredOrders = allOrders.filter((order: Order) => {
			...
			return claims.owners.includes(sellerId) || claims.moderators.includes(sellerId) || claims.staffs.includes(sellerId);
		});
